...
GpsImuBatchFuser::GpsImuBatchFuser(
    ...
    , dt_{imu_integration_interval_sec}
{
  ...
  graph_.addPrior(X(0), prior_pose, prior_pose_noise_model);
  graph_.addPrior(V(0), prior_velocity, prior_velocity_noise_model);
  graph_.addPrior(B(0), prior_imu_bias, prior_bias_noise_model);
  ...
  last_estimate_ = initial_estimate_;
...
void GpsImuBatchFuser::addGpsData(const nav::NavlabGPS& gps, bool do_plot_update)
{
  ...
  initial_estimate_ = last_estimate_;
  ...
  initial_estimate_.insert(X(curr_state_num_), predicted_state.pose());
  initial_estimate_.insert(V(curr_state_num_), predicted_state.v());
  initial_estimate_.insert(B(curr_state_num_), prev_bias_state);

  // Estimate all states.
  LevenbergMarquardtOptimizer optimizer(graph_, initial_estimate_);
  last_estimate_ = optimizer.optimize();
...
void GpsImuBatchFuser::addImuData(const nav::NavlabIMU& imu)
{
