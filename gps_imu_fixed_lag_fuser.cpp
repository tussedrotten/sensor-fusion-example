...
GpsImuFixedLagFuser::GpsImuFixedLagFuser(
    ...
    , dt_{imu_integration_interval_sec}
{
  ...
  smoother_ = std::make_shared<IncrementalFixedLagSmoother>(lag, parameters);
  ...
  // Insert timestamp for prior.
  FixedLagSmoother::KeyTimestampMap initial_timestamps;
  initial_timestamps[X(0)] = init_gps.time;
  initial_timestamps[V(0)] = init_gps.time;
  initial_timestamps[B(0)] = init_gps.time;

  // Add priors to ISAM2.
  smoother_->update(graph, initial_estimate, initial_timestamps);
...
void GpsImuFixedLagFuser::addGpsData(const nav::NavlabGPS& gps, bool do_plot_update)
{
  ...
  newTimestamps[X(curr_state_num_)] = gps.time;
  newTimestamps[V(curr_state_num_)] = gps.time;
  newTimestamps[B(curr_state_num_)] = gps.time;

  // Estimate all states.
  smoother_->update(graph, initial_estimate, newTimestamps);
  for (int i=0; i < 10; ++i)
  {
    smoother_->update();
  }
  last_estimate_ = smoother_->calculateEstimate();
...
//  std::cout << "Standard deviations for X(" << curr_state_num_ << "): " << stddevs.transpose() << std::endl;
  std::cout << "Number of timestamps: " << smoother_->timestamps().size() << std::endl;
...
void GpsImuFixedLagFuser::addImuData(const nav::NavlabIMU& imu)
{
